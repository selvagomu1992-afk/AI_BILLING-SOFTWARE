...
const StatusBudge = ({ status = "", size = "default", showIcon = true }) => {
    ...
    return (
        ...
        ease-out hover:scale-105 hover-shadow-sm group relative overflow-hidden`}>
            <div className={`absolute inset-0 bg-liner-to-r ${config.gradient} opacity-0
            ...
            <span className="relative z-10 font-semibold tracking-wide file-letter:upperCase">
                {s === "default" ? status : s}
...
// Status With Cound
export const StatuswithCount = ({ status, count, size = "default", showIcon = true }) => {
    return (
        <div className='Inline-flex items-center gap-2 group'>
            <StatusBudge status={status} size={size} showIcon={showIcon} />
            {count !== undefined && (
                <span className={`font-semibold ${size === "small" ? "text-xs" : size === "large" ? "text-base" : "text-sm"}`}>
                    {count}
                </span>
            )}
        </div>
